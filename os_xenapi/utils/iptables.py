...
def configure_himn_forwards(forwarding_interfaces, dom0_himn_ip):
    ...
    for interface in forwarding_interfaces:
        ...
        cmd_tail = ['POSTROUTING', '-t', 'nat', '-o', interface, '-j',
                    'MASQUERADE']
        set_domu_iptables(cmd_tail)

        cmd_tail = ['FORWARD', '-i', interface, '-o', eth, '-m', 'state',
                    '--state', 'RELATED,ESTABLISHED', '-j', 'ACCEPT']
        set_domu_iptables(cmd_tail)

        cmd_tail = ['FORWARD', '-i', eth, '-o', interface, '-j', 'ACCEPT']
        set_domu_iptables(cmd_tail)

        cmd_tail = ['INPUT', '-i', eth, '-j', 'ACCEPT']
        set_domu_iptables(cmd_tail)
...
def set_domu_iptables(cmd_tail):
    cmd_check = ['iptables', '-C', ] + cmd_tail
    cmd_excute = ['iptables', '-A', ] + cmd_tail
    try:
