...
# Check whether install released os-xenapi or install with upstream
function patch_os_xenapi {
    # TODO (huanxie): maybe use the other way, will check later
    echo $PATCH_OS_XENAPI
    return 0
}

# Get the path that os-xenapi is install by pypi
function python_sys_path {
    # TODO (huanxie): need return according to python version, not fixed
...
function install_plugins {
    ...
    dom0_plugin_dir=$(dom0_plugin_location)
    local local_plugin_dir
    if [ "0" = "$PATCH_OS_XENAPI" ]; then
        local_plugin_dir=$(python_sys_path)
    else
        local_plugin_dir=$OS_XENAPI_DIR
    fi
    tar -czf - -C $local_plugin_dir/os_xenapi/dom0/etc/xapi.d/plugins/ ./ |
