...
class VBDTestCase(base.TestCase):
    ...
    @mock.patch.object(lockutils, "synchronized_with_prefix")
    def test_vbd_plug_check_synchronized(self, mock_synchronized_with_prefix):
        self.session.VBD.plug("vbd_ref", "vm_ref")
        mock_synchronized_with_prefix.assert_called_once_with("xenapi-vbd-vm_ref")

    @mock.patch.object(lockutils, "synchronized_with_prefix")
    def test_vbd_unplug_check_synchronized(self, mock_synchronized_with_prefix):
        self.session.VBD.unplug("vbd_ref", "vm_ref")
        mock_synchronized_with_prefix.assert_called_once_with("xenapi-vbd-vm_ref")
