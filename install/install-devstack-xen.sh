...
OS_XENAPI_SRC="https://github.com/openstack/os-xenapi/archive/master.zip"
TEST_TYPE="none"
...
GUEST_IP=\$(. "$COMM_DIR/functions" && find_ip_by_name DevStackOSDomU 0)
if [ -n \$GUEST_IP ]; then
ssh -q \
    -o Batchmode=yes \
    -o StrictHostKeyChecking=no \
...
DOM0_SCRIPT_DIR="$DOM0_INSTALL_DIR/scripts"
DOM0_COMM_DIR="$DOM0_INSTALL_DIR/common"
DOM0_CONF_DIR="$DOM0_INSTALL_DIR/conf"

DOMU_OPT_DIR="/opt/stack"
DOMU_DEV_STACK_DIR="$DOMU_OPT_DIR/devstack"
...
# XenAPI specific
XENAPI_CONNECTION_IP="$XENSERVER_IP"
...
echo "$DOM0_OPT_DIR"
echo "$DOM0_OS_API_DIR"
echo "$DOM0_INSTALL_DIR"
echo "$DOM0_SCRIPT_DIR"
echo "$DOM0_COMM_DIR"
echo "$DOM0_CONF_DIR"
...
cd $DOM0_OPT_DIR
rm -f master.zip
rm -rf $DOM0_OS_API_DIR
wget --no-check-certificate "$OS_XENAPI_SRC"
unzip -o master.zip -d $DOM0_OS_API_DIR
mv $DOM0_OS_API_DIR/os-xenapi-*/* $DOM0_OS_API_DIR
rm $DOM0_OS_API_DIR/os-xenapi-* -rf
if [ ! -d "$DOM0_INSTALL_DIR" ]; then
    cp -rf /tmp/install $DOM0_OS_API_DIR
fi
cd $DOM0_INSTALL_DIR

# create template if needed
$DOM0_INSTALL_DIR/create_ubuntu_template.sh "$XENAPI_CONNECTION_IP"
...
$DOM0_INSTALL_DIR/config_devstack_domu_vm.sh "$XENAPI_CONNECTION_IP"
# install DevStack on the VM



#./install_devstack_domu.sh "$XENAPI_CONNECTION_IP"
...
GUEST_IP=\$(. "$DOM0_COMM_DIR/functions" && find_ip_by_name DevStackOSDomU 0)
ssh -q \
    -o Batchmode=yes \
    -o StrictHostKeyChecking=no \
    -o UserKnownHostsFile=/dev/null \
...
echo "logon to DomU, install devstack"
cd $DOMU_OPT_DIR
echo "copy devstack source code"
rm -f master
wget --no-check-certificate "$DEVSTACK_SRC"
mkdir -p $DOMU_DEV_STACK_DIR
unzip -o master -d $DOMU_DEV_STACK_DIR
cp local.conf $DOMU_DEV_STACK_DIR/*/
mv $DOMU_DEV_STACK_DIR/*/* $DOMU_DEV_STACK_DIR/
rm $DOMU_DEV_STACK_DIR/openstack-dev-* -rf
