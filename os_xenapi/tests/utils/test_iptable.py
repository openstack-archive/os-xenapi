...
class XenapiIptableTestCase(base.TestCase):
    def test_configure_dom0_iptables(self):
        ...
        expect_cmd4 = 'service iptables save'
        expect_calls = [mock.call(expect_cmd1), mock.call(expect_cmd2),
                        mock.call(expect_cmd3), mock.call(expect_cmd4)]
        iptables.configure_dom0_iptables(client)
        client.ssh.assert_has_calls(expect_calls)

    def test_configure_dom0_iptables_invalid_session(self):
        client = None

        self.assertRaises(exception.IptableClientInvalid,
                          iptables.configure_dom0_iptables, client)
    ...
    def test_configure_himn_forwards(self, mock_excute, mock_get_eth):
        ...
        expect_call1 = mock.call('/usr/bin/touch', '/tmp/test_by_himn')
        expect_call2 = mock.call('iptables', '-t', 'nat', '-A', 'POSTROUTING',
                                 '-o', fack_end_point[0], '-j', 'MASQUERADE')
        expect_call3 = mock.call('iptables', '-t', 'nat', '-A', 'POSTROUTING',
        ...
        expect_call6 = mock.call('iptables', '-A', 'FORWARD',
                                 ...
                                 '-j', 'ACCEPT')
        expect_call7 = mock.call('iptables', '-A', 'FORWARD',
                                 '-i', 'fake_eth', '-o', fack_end_point[1],
                                 '-j', 'ACCEPT')
        expect_call8 = mock.call('iptables', '-A', 'INPUT', '-i', 'fake_eth',
                                 '-j', 'ACCEPT')
        expect_call9 = mock.call('iptables', '-t', 'filter', '-S', 'FORWARD')
        expect_call10 = mock.call('iptables', '-t', 'nat', '-S', 'POSTROUTING')

        expect_calls = [expect_call1, expect_call2,
                        expect_call4, expect_call6,
                        expect_call8, expect_call9,
                        expect_call10, expect_call1,
                        expect_call3, expect_call5,
    ...
    def test_configure_himn_forwards_no_eth_exc(self, mock_excute,
        ...
        self.assertRaises(exception.IptableNoNetworkInterface,
