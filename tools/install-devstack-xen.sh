...
optional arguments:
 ...
 -c LOCAL_PACK_DIR     Priority use the local install package if this value has been set
...
NODE_NAME=""
CONTROLLER_IP=""
LOCAL_PACK_DIR="/root/os-xen-local"
...
set +e
while getopts ":t:d:fnl:j:e:o:s:w:a:i:m:c:" flag; do
...
copy_logs_on_failure on_xenserver << END_OF_XENSERVER_COMMANDS
    ...
    if [ -d  "$LOCAL_PACK_DIR" ]; then
        DOM0_OS_API_DIR="$LOCAL_PACK_DIR/os-xenapi"
    else
