...
LOGDIR=${LOGDIR} DEVSTACK_SRC=$DEVSTACK_SRC ./launch_devstack.sh $XENSERVER_PASS
...
if [ "$TEST_TYPE" != "none" ]; then
    if [ ! -e ~/.ssh/id_rsa.pub ]; then
        echo "Error, could't confirm the devstack install status, tests request failed"
    else
        echo "Warning, user ask to do tests after installation, force waiting for installation finish"
        WAIT_TILL_LAUNCH=1
        COPYENV=1
    fi
fi
