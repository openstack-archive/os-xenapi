...
optional arguments:
 -t TEST_TYPE          Type of the tests to run. One of [none, exercise, smoke, full]
                       defaults to none
 -c OSXENAPI_SRC_LOCAL Priority use the local os-xenapi install repository if this value
                       has been set
...
OSXENAPI_SRC=${OSXENAPI_SRC:-"https://github.com/openstack/os-xenapi/archive/master.zip"}
TEST_TYPE="none"
...
set +e
while getopts ":t:d:fnrl:j:e:o:s:w:a:i:m:c:" flag; do
...
copy_logs_on_failure on_xenserver << END_OF_XENSERVER_COMMANDS
    ...
    if [ -d  "$OSXENAPI_SRC_LOCAL" ]; then
        DOM0_OS_API_DIR="$OSXENAPI_SRC_LOCAL/os-xenapi"
    else
        DOM0_OS_API_UNZIP_DIR="$DOM0_OPT_DIR/os-xenapi"
        DOM0_OS_API_DIR="\$DOM0_OS_API_UNZIP_DIR/os-xenapi-*"
        mkdir $DOM0_OPT_DIR
        cd $DOM0_OPT_DIR
        wget --no-check-certificate "$OSXENAPI_SRC"
