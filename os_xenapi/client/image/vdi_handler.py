...
class ImageStreamToVDIs(object):
    ...
    def _clean(self):
        if self.task_ref:
            self.session.call_xenapi("task.destroy", self.task_ref)

    def start(self):
        label = 'VDI_IMPORT_for_' + self.instance['name']
        desc = 'Importing VDI for instance: %s' % self.instance['name']
        self.task_ref = self.session.call_xenapi("task.create", label,
                                                 desc)
        try:
            with tarfile.open(mode="r|gz", fileobj=self.image_stream) as tar:
                for vhd in tar:
                    ...
                    self._vhd_stream_to_vdi(vhd_file_parser, vdi_ref,
                                            file_size)
                    vdi_uuid = self.session.call_xenapi('VDI.get_uuid',
                                                        vdi_ref)
