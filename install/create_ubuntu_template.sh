...
TNAME="jeos_template_for_unbuntu"
SNAME_TEMPLATE="jeos_snapshot_for_unbuntu"
...
if [ -z "$templateuuid" ]; then
    ...
    if [ -z "$PRESEED_URL" ]; then
        ...
        cp -f $THIS_DIR/ubuntupreseed.cfg $HTTP_SERVER_LOCATION
        cp -f $THIS_DIR/ubuntu_latecommand.sh $HTTP_SERVER_LOCATION/latecommand.sh
