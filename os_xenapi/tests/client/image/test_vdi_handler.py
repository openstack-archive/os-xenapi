...
class ImageStreamToVDIsTestCase(base.TestCase):
    ...
    def test_get_virtual_size(self):
        ...
        header_data = '\x00' * 48 + struct.pack('!Q', ONE_GB)
    ...
    def test_vhd_stream_to_vdi(self, conn_close, conn_getRes, conn_send,
        ...
        image_cmd = vdi_handler.ImageStreamToVDIs(self.context, self.session,
                                                  self.instance, self.host_url,
                                                  self.stream)
        image_cmd.cache = '\x00' * cache_size
...
class VdisToTarStreamTestCase(base.TestCase):
    ...
    def test_get_vhd_size(self):
        ...
        DATA_FOOTER_AND_DDH = '\x00' * SIZE_OF_FOOTER
        DATA_FOOTER_AND_DDH += '\x00' * 16 + DDH_BAT_OFFSET + \
                               '\x00' * 4 + DDH_MAX_BAT_ENTRIES + \
                               DDH_BLOCK_SIZE
        DATA_FOOTER_AND_DDH += '\x00' * (SIZE_OF_DDH - 36)

        # Construct the padding bytes
        DATA_PADDING = '\x00' * SIZE_PADDING
        ...
        # ignored.
        DATA_BAT = '\x00\x00\x00\x08\x00\x00\x50\x0d\xff\xff\xff\xff' + \
                   '\x00\x00\x10\x09\x00\x00\x20\x0a\x00\x00\x30\x0b' + \
                   '\x00\x00\x40\x0c\xff\xff\xff\xff\x00\x00\x60\x0e' + \
                   '\x00\x00\x70\x0f\x00\x00\x80\x10\x00\x00\x90\x11' + \
                   '\x00\x00\xa0\x12\x00\x00\xb0\x13\x00\x00\xc0\x14' + \
                   '\x00\x00\xd0\x15' + \
                   '\xff\xff\xff\xff' * (MAX_BAT_ENTRIES - 16)
