...
class XenapiIptableTestCase(base.TestCase):
    ...
    @mock.patch.object(iptables, 'execute_dom0_commands')
    def test_configure_dom0_iptables(self, fake_execute):
        ...
        expect_call3 = mock.call(client, ('iptables -t filter -C %s -p tcp -m '
                                          ...
                                          % xs_chain, ))
        expect_call4 = mock.call(client, ('iptables -t filter -I %s -p tcp -m '
                                          'tcp --dport 6640 -j ACCEPT'
                                          % xs_chain, ))
    ...
    def test_configure_himn_forwards(self, mock_execute, mock_get_eth):
        ...
        expect_call3 = mock.call('iptables', '-C', 'POSTROUTING', '-t', 'nat',
                                 '-o', fack_end_point[0], '-j', 'MASQUERADE')
        expect_call4 = mock.call('iptables', '-A', 'POSTROUTING', '-t', 'nat',
                                 '-o', fack_end_point[0], '-j', 'MASQUERADE')

        expect_call5 = mock.call('iptables', '-C', 'POSTROUTING', '-t', 'nat',
                                 '-o', fack_end_point[1], '-j', 'MASQUERADE')
        expect_call6 = mock.call('iptables', '-A', 'POSTROUTING', '-t', 'nat',
                                 '-o', fack_end_point[1], '-j', 'MASQUERADE')
    ...
    def test_execute_dom0_commands(self):
        # Execute first command and continue based on first command result
        client = mock.Mock()
        fake_cmd_list = ['fake_cmd1', 'fake_cmd2', 'fake_cmd3']
        expect_ssh_calls = [mock.call('fake_cmd1'),
                            mock.call('fake_cmd2'),
                            mock.call('fake_cmd3')]

        iptables.execute_dom0_commands(client, fake_cmd_list)
        client.ssh.assert_has_calls(expect_ssh_calls)
