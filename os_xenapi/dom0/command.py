...
def write_to_xenstore(session, path, value, dom_id):
    args = {'dom_id': dom_id, 'path': path, 'value': value}
    return session.call_plugin('xenstore.py', 'write_record', args)


def read_from_xenstore(session, path, dom_id, ignore_missing_path=True):
...
def delete_from_xenstore(session, path, dom_id):
    args = {'dom_id': dom_id, 'path': path}
