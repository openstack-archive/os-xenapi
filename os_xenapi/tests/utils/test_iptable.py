...
class XenapiIptableTestCase(base.TestCase):
    ...
    @mock.patch.object(sshclient, 'SSHClient')
    @mock.patch.object(iptables, 'exit_with_usage')
    @mock.patch.object(iptables, 'execute_dom0_iptables_commands')
    def test_configure_dom0_iptables_invalid_session(self, fake_execute,
                                                     fake_exit, fake_client):
        fake_client.side_effect = fake_client_exception
        iptables.configure_dom0_iptables('fake_dom0_himn_ip', 'fake_user_name',
                                         'fake_password')
        fake_client.assert_called_once_with('fake_dom0_himn_ip',
                                            'fake_user_name',
                                            'fake_password')
