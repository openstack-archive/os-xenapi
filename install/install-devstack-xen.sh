...
rm -f $TMPDIR/jeos-for-devstack.xva
xe template-export template-uuid="\$JEOS_TEMPLATE" filename="$TMPDIR/jeos-for-devstack.xva" compress=true
END_OF_EXPORT_COMMANDS
    echo "OK"

    echo -n "Copy exported template to local file..."
    if scp -3 $_SSH_OPTIONS "root@$XENSERVER:$TMPDIR/jeos-for-devstack.xva" "$JEOS_FILENAME"; then
        echo "OK"
        RETURN_CODE=0
    else
        echo "FAILED"
...
rm -f $TMPDIR/jeos-for-devstack.xva
echo "  downloading $JEOS_URL to $TMPDIR/jeos-for-devstack.xva"
wget -qO $TMPDIR/jeos-for-devstack.xva "$JEOS_URL"
echo "  importing $TMPDIR/jeos-for-devstack.xva"
xe vm-import filename=$TMPDIR/jeos-for-devstack.xva
rm -f $TMPDIR/jeos-for-devstack.xva
...
copy_logs_on_failure on_xenserver << END_OF_XENSERVER_COMMANDS
    ...
    cd $DOM0_INSTALL_DIR
    LOGDIR=$LOGDIR TEST_TYPE=$TEST_TYPE ./install_on_xen_host.sh $XENSERVER_PASS
    rm $TMPDIR -rf
END_OF_XENSERVER_COMMANDS
