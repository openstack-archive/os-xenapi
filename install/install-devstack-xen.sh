...
WAIT_TILL_LAUNCH=${"$1":-1}
shift || echo "use default WAIT_TILL_LAUNCH value[$WAIT_TILL_LAUNCH]!"
set -u
...
# will remove after file submit
    if [ ! -f "$DOM0_INSTALL_DIR/install_on_xen_host.sh" ]; then
        cp -rf /tmp/install $DOM0_OS_API_DIR
    fi
    cd $DOM0_INSTALL_DIR
    LOGDIR=$LOGDIR WAIT_TILL_LAUNCH=$WAIT_TILL_LAUNCH ./install_on_xen_host.sh $XENSERVER_PASS
    #never
END_OF_XENSERVER_COMMANDS
