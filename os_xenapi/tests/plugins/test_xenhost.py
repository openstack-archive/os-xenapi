...
class XenHostTestCase(plugin_test.PluginTestBase):
    ...
    def test_set_config_remove_none_key(self, mock_json_loads):
        ...
        self.mock_patch_object(self.host,
                               '_write_config_dict',
                               'fake_write_config_dict_retsurn')
...
'''
# Todo: need a proper way to mock local variable functions
...
self.mock_patch_object(self.host.XenAPI.Session, mock.Mock, 'fake'
        self.mock_patch_object(self.host,
                               '_resume_compute')
    ...
    def test_host_join_force_join(self):
        temp_arg_dict = {'force': 'true',
                         ...
                         'master_pass': 'fake_master_pass',
                         'compute_uuid': 'fake_compute_uuid'}
        self.mock_patch_object(self.session,
                               'login_with_password',
                               'fake_login_with_password')
        self.mock_patch_object(self.session.xenapi.VM,
                               'get_by_uuid',
                               'fake_compute_ref')
        self.mock_patch_object(self.session.xenapi.VM,
    ...
    def test_write_config_dict(self):
        temp_arg_dict = 'fake_arg_dict'
        fake_file = mock_open()
        with mock.patch.object(six.moves.builtins, "open", fake_file,
                               create=True):
            with open('self.host.config_file_path', 'w') as fake_handle:
                self.host._write_config_dict(temp_arg_dict)
                self.assertEqual(fake_handle.read(), temp_arg_dict)

    def test_get_config_dict(self):
        temp_arg_dict = 'fake_arg_dict'
        fake_file = mock_open(read_data=temp_arg_dict)
        with mock.patch.object(six.moves.builtins, "open", fake_file,
                               create=True):

            config_dict_return = self.host._get_config_dict()
            self.assertEqual(config_dict_return, temp_arg_dict)

    def test_get_config_dict_IOError(self):
        self.host.config_file_path = '/usr/etc/not_exist.conf'
        self.mock_patch_object(self.host,
                               '_write_config_dict',
                               'fake_write_config_dict_return')

        conf_dict_return = self.host._get_config_dict()
        self.assertEqual(conf_dict_return, {})
