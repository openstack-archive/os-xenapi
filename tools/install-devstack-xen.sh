...
optional arguments:
 ...
 -s SUPP_PACK_URL      URL to a supplemental pack that will be installed on the host
                       ...
                       picked up.
 -o OS_XENAPI_SRC      It can be a local repository or an URL. User can pass a local
...
# Got install repositories. For os-xenapi, it is small and only need to move to
# xenserver, we will download to current client if input OS_XENAPI_SRC is an
# URL. For devstack, it is much bigger and need move twice(current client to
# xenserver and xenserver to DomU), it would use too much time if we download it
# to current client and move it. So we will download it by devstack.service on
# DomU when DomU first bootup
