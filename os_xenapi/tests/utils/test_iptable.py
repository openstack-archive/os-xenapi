...
class XenapiIptableTestCase(base.TestCase):
    ...
    def test_configure_himn_forwards(self, mock_execute, mock_get_eth):
        ...
        expect_call5 = mock.call('iptables', '-t', 'filter', '-A', 'FORWARD',
                                 '-i', fack_end_point[0], '-o', 'fake_eth',
                                 '-m', 'state', '--state',
                                 'RELATED,ESTABLISHED', '-j', 'ACCEPT')

        expect_call6 = mock.call('iptables', '-t', 'filter', '-A', 'FORWARD',
                                 '-i', fack_end_point[1], '-o', 'fake_eth',
                                 '-m', 'state', '--state',
                                 'RELATED,ESTABLISHED', '-j', 'ACCEPT')

        expect_call7 = mock.call('iptables', '-t', 'filter', '-A', 'FORWARD',
                                 '-i', 'fake_eth', '-o', fack_end_point[0],
                                 '-j', 'ACCEPT')

        expect_call8 = mock.call('iptables', '-t', 'filter', '-A', 'FORWARD',
                                 '-i', 'fake_eth', '-o', fack_end_point[1],
                                 '-j', 'ACCEPT')

        expect_call9 = mock.call('iptables', '-t', 'filter', '-A', 'INPUT',
                                 '-i', 'fake_eth', '-j', 'ACCEPT')

        expect_call10 = mock.call('iptables', '-t', 'filter', '-S', 'FORWARD')
        expect_call11 = mock.call('iptables', '-t', 'nat', '-S', 'POSTROUTING')
        expect_call12 = mock.call('service', 'iptables-persistent', 'save')

        expect_calls = [expect_call1, expect_call2,
                        expect_call3, expect_call5,
                        expect_call7, expect_call4,
                        expect_call6, expect_call8,
