...
class GlanceTestCase(plugin_test.PluginTestBase):
    ...
    def test_create_connection_https(self, mock_HTTPConn):
        ...
        mock_HTTPConn.connect.asset_called_once()
    ...
    def test_download_and_verify_ok(self, mock_urlopen):
        ...
        mock_urlopen.asset_called_with(fake_request)
        mock_extract_tarball.asset_called_once()
        mock_md5_new.asset_called_once()
        mock_md5_new.hexdigest.asset_called_once()
    ...
    def test_download_tarball_by_url_v1(self, mock_request):
        ...
        expected_url = "%(glance_endpoint)s/v1/images/%(image_id)s/file" % {
        ...
        mock_request.asset_called_with(expected_url, 'fake_extra_headers')
        mock_download_tarball_and_verify.asset_called_with('fake_request',
                                                           'fake_staging_path')
    ...
    def test_upload_tarball_by_url_http_v1(self):
        mock_HTTPConn = self.mock_patch_object(
            self.glance, '_create_connection', mock.Mock())
    ...
    def test_update_image_meta_ok_v2(self):
        ...
        fake_properties = {'fake_path': 'fake_value'}
        new_fake_properties = {'key': '/fake-path',
                               'value': 'fake_value',
                               'op': 'fake_op'}
        fake_body = [
        ...
        self.mock_patch_object(fake_conn, 'request')
        self.mock_patch_object(fake_conn, 'getresponse', mock.Mock())
        fake_conn.getresponse().status = httplib.OK

        self.glance._update_image_meta_v2(fake_conn, 'fake_image_id',
                                          fake_extra_headers, fake_properties)
        fake_conn.Request.asset_called_with('PATCH',
    ...
    def test_validate_image_status_before_upload_failed_v1(self):
        mock_conn = mock.Mock()
        fake_url = 'http://fake_host/fake_path/fake_image_id'
        mock_head_resp = mock.Mock()
    ...
    def test_download_vhd2_v1(self):
        ...
        mock_make_staging_area.asset_called_with('fake_sr_path')
        mock_download_tarball_by_url.asset_called_with('fake_sr_path',
                                                       'fake_staging_path',
                                                       'fake_image_id',
                                                       'fake_endpoint',
                                                       'fake_extra_headers')
        mock_import_vhds.asset_called_with('fake_sr_path',
                                           'fake_staging_path',
                                           'fake_uuid_stack')
        mock_cleanup_staging_area.asset_called_with('fake_staging_path')
    ...
    def test_upload_vhd2_v1(self):
        ...
        mock_make_staging_area.asset_called_with('fake_sr_path')
        mock_upload_tarball_by_url.asset_called_with('fake_sr_path',
                                                     'fake_staging_path',
                                                     'fake_image_id',
                                                     'fake_endpoint',
                                                     'fake_extra_headers')
        mock_prepare_staging_area.asset_called_with('fake_sr_path',
                                                    'fake_staging_path',
                                                    'fake_vid_uuids')
        mock_cleanup_staging_area.asset_called_with('fake_staging_path')
