...
$THIS_DIR/prepare_guest_template.sh "$DEV_STACK_DOMU_NAME"
...
#
$THIS_DIR/save_networks_conf.sh "$DEV_STACK_DOMU_NAME"
...
if [ $HOST_IP_IFACE == "eth${MGT_DEV_NR}" ]; then
    ...
    if [ $MGT_IP == "dhcp" ]; then
        OS_VM_SERVICES_ADDRESS=$(find_ip_by_name $DEV_STACK_DOMU_NAME $MGT_DEV_NR)
    fi
else
    OS_VM_SERVICES_ADDRESS=$PUB_IP
    if [ $PUB_IP == "dhcp" ]; then
        OS_VM_SERVICES_ADDRESS=$(find_ip_by_name $DEV_STACK_DOMU_NAME $PUB_DEV_NR)
    fi
fi
