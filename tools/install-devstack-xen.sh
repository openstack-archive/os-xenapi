...
optional arguments:
 -t TEST_TYPE          Type of the tests to run. One of [none, exercise, smoke, full]
                       defaults to none
 -c DEVSTACK_SRC_LOCAL Priority use the local os-xenapi install repository if this value
                       has been set
...
copy_logs_on_failure on_xenserver << END_OF_XENSERVER_COMMANDS
    ...
    if [ -d  "$DEVSTACK_SRC_LOCAL" ]; then
        DOM0_OS_API_DIR="$DEVSTACK_SRC_LOCAL/os-xenapi"
    else
