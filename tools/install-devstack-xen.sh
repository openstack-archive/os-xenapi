...
# If input repository is URL, for os-xenapi, it is only needed on xenserver, so
# we will download it and move it to xenserver when needed; for devstack, it
# is needed on DomU, so we configure a service on DomU and download it when
# DomU first bootup.
