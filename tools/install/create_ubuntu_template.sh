...
if have_multiple_hosts; then
    cat >&2 << EOF
Warnning: multiple hosts found. This might mean that the XenServer is a member
of a pool.
EOF
fi
...
#
templateuuid=$(xe template-list possible-hosts=$uuid name-label="$TNAME")
...
template_uuid=$(xe_min template-list possible-hosts=$uuid name-label="$TNAME")
