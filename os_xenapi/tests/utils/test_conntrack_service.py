...
class XenapiConntrackServiceTestCase(base.TestCase):
    def test_ensure_conntrack_packages(self):
        ...
        cmd = "if ! yum list installed conntrack-tools; then\n"  \
              ...
              "    yum install -y -c /tmp/repo/CentOS-Base.repo " \
              "--enablerepo=base        --releasever=$centos_ver " \
              "conntrack-tools;\n" \
              "    rm -rf TMP_REPO_DIR;\n" \
              "fi\n"
    ...
    def test_enable_conntrack_service(self, mock_ensure_conntrack):
        client = mock.Mock()
        client.ssh.side_effect = [['fake_version', None],
                                  ['fake_conf_list', None],
                                  None, None, None]
        ...
        expect_call1 = mock.call("yum info conntrack-tools | grep '^Version' |"
                                 " awk '{print $3}')")
        expect_call2 = mock.call("find /usr/share/doc/conntrack-tools-" +
                                 "fake_version -name conntrackd.conf | grep "
                                 "stats")
        ...
        expect_cmd = "cat >/etc/logrotate.d/conntrackd <<EOF\n" \
                     ...
                     "}\n" \
                     "EOF"
        expect_call4 = mock.call(expect_cmd)
        expect_call5 = mock.call('service conntrackd restart')
    ...
    def test_enable_conntrack_service_no_config(self, mock_ensure_conntrack):
        client = mock.Mock()
        client.ssh.side_effect = [['fake_version', None], [None, None]]
        expect_cmd1 = mock.call("yum info conntrack-tools | grep '^Version' |"
                                " awk '{print $3}')")
        expect_cmd2 = mock.call("find "
                                "/usr/share/doc/conntrack-tools-fake_version"
                                " -name conntrackd.conf | grep stats")

        self.assertRaises(exception.NotFound,
                          conntrack_service.enable_conntrack_service,
                          client)
        client.ssh.assert_has_calls([expect_cmd1, expect_cmd2])
