...
class SshExecCmdFailure(OsXenApiException):
    msg_fmt = _("Failed to execute the command: %(command)s\n"
                "stdout: %(stdout)s\n"
                "stderr: %(stderr)s")
...
class SSHClient(object):
    ...
    def ssh(self, command, get_pty=True, output=False):
        if self.log:
            self.log.debug("START executing command: [%s]" % command)
        ...
        if self.log:
            self.log.debug("SUCCESSFULLY executing command: [%s]" % command)
        return out, err
