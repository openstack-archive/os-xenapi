...
class XenapiPluginTestCase(base.TestCase):
    ...
    def test_get_os_xenapi_dir_with_version(self, mock_exec, mock_tmp):
        VERSION = '0.3.1'
        ret_val = xapi_plugin.get_os_xenapi_dir(VERSION)

        self.assertEqual(ret_val, '/tmp/tmp1VyeJn')
    ...
    def test_get_os_xenapi_dir_no_version(self, mock_exec, mock_tmp):
        ...
        ret_val = xapi_plugin.get_os_xenapi_dir()

        self.assertEqual(ret_val, fake_loc)
    ...
    @mock.patch.object(common_function, 'execute')
    def test_install_plugins_to_dom0_no_version(self, mock_exec, mock_get):
        FAKE_PKG_PATH = '/fake/pkg/path'
        ssh_client = mock.Mock()
        mock_get.return_value = FAKE_PKG_PATH
        mock_exec.side_effect = ['file1\nfile2',  # For cmd: find
                                 '',  # For cmd: rm
                                 ]
        ...
        # Shouldn't invoke mock_exec to remove the package dir.
        remove_call = mock.call('rm', '-rf', FAKE_PKG_PATH)
        self.assertTrue(remove_call not in mock_exec.call_list())

    @mock.patch.object(xapi_plugin, 'get_os_xenapi_dir')
    @mock.patch.object(common_function, 'execute')
    def test_install_plugins_to_dom0_with_version(self, mock_exec, mock_get):
        VERSION = '0.3.1'
        ...
        self.assertEqual(scp_expect, ssh_client.scp.call_args_list)
        self.assertEqual(ssh_expect, ssh_client.ssh.call_args_list)
        # Should invoke mock_exec to remove the package dir.
        remove_call = mock.call('rm', '-rf', FAKE_PKG_PATH)
        mock_exec.has_calls([remove_call])
